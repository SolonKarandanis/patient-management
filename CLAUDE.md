...
- `ai-service` — Spring AI 2.0.0, Spring Boot 4 / WebFlux (reactive). Multi-provider LLM support (Anthropic, OpenAI, Gemini) selected via `ai.provider` property. JDBC-backed chat memory (`spring_ai_chat_memory` table) with a sliding window of 50 messages. pgvector RAG store. Hexagonal architecture: `domain/` (model + `LlmPort` port), `infrastructure/` (single `SpringAiAdapter` + JDBC memory repo — see below). Uses its own Postgres at `192.168.1.6:**5433**/aiservice` (port 5433, not 5432 — separate PG18 instance for pgvector). Maven-built. **Not exposed through the gateway** — auth-service calls it directly via `AiServiceClient` (RestTemplate).
...
**AI / LLM layer (ai-service):** `ChatService` (`service/ChatService.java`) is a thin pass-through — `chat()`/`streamChat()` delegate straight to `LlmPort`; it only touches `ChatMemory` directly for auxiliary reads (`getSessionTurnCount`, `clearSession`, `sessionExists`, `getConversationHistory`). The actual conversation loop lives inside `infrastructure/springai/SpringAiAdapter.java`, the sole `LlmPort` implementation: it wraps one Spring AI `ChatClient` (built by `config/ChatModelRegistrar.java`, which switches on `ai.provider` to construct exactly one `AnthropicChatModel`/`OpenAiChatModel`/`GoogleGenAiChatModel` bean — there are no separate per-provider adapter classes) with `defaultAdvisors(MessageChatMemoryAdvisor, QuestionAnswerAdvisor)`; the memory advisor transparently loads/saves history keyed by `chat_memory_conversation_id` (bound to `sessionId`), so no manual append step exists anywhere. `MessageWindowChatMemory` (configured in `config/ChatMemoryConfig.java`, `maxMessages(50)`) keeps the last 50 messages per session using the custom `infrastructure/memory/JdbcChatMemoryRepository` against the `spring_ai_chat_memory` table. `ArticleController` is a separate, stateless blog-post generator that builds its own `ChatClient` directly with no advisors (no session memory). The RAG vector store (pgvector, HNSW, cosine, 1536-dim) uses OpenAI `text-embedding-3-small` for embeddings regardless of which chat provider is active. **Prompt caching (Anthropic `cache_control`) is not enabled** — Spring AI 2.0.0 exposes it via `AnthropicChatOptions.builder().cacheOptions(AnthropicCacheOptions...)` (strategies: `NONE`/`TOOLS_ONLY`/`SYSTEM_ONLY`/`SYSTEM_AND_TOOLS`/`CONVERSATION_HISTORY`), but `ChatModelRegistrar` never sets it, so it defaults to `NONE`.
...
- **ai-service is also hexagonal:** `LlmPort` in `domain/port/` is the sole seam between `ChatService` and the LLM backends. Its only implementation is `SpringAiAdapter`, which wraps whichever Spring AI `ChatModel` `ChatModelRegistrar` (an `ImportBeanDefinitionRegistrar`) constructs based on the `ai.provider` property — there is no per-provider adapter class or `@ConditionalOnProperty` switch. Swap provider by changing `ai.provider` in `application.properties` — no code changes needed.
